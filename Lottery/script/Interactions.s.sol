...
contract FundSubscription is Script, CodeConstants {
    ...
    function fundSubscriptionUsingConfig() public {
        ...
        address account = helperConfig.getConfig().account;
        fundSubscription(vrfCoordinator, subscribId, linkToken, account);
    }

    function fundSubscription(address vrfCoordinator, uint256 subscribId, address linkToken, address account) public {
        ...
        if (block.chainid == CodeConstants.LOCAL_CHAIN_ID) {
            vm.startBroadcast();
            VRFCoordinatorV2_5Mock(vrfCoordinator).fundSubscription(subscribId, FUND_AMOUNT * 100);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast(account);
